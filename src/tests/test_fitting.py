...
def test_SDA_for_Y_lm_distribution():
    ...
    for L_max, evens, rand_dist in [(1, False, False), (2, True, False), (3, False, True), (2, True, True)]:
        for trial in range(1):
            ...
            print("------------------\nrunning SDA trial = ", trial)
            estimate_B_lms = fitting.SDA_Y_lms_distribution(samples_xyz, L_max, 0.0001, 1000, .1, evens)
            est_probs = fitting.Y_lms_distribution(angles_array[:,0], angles_array[:,1], L_max, estimate_B_lms, evens)
            est_probs /= sum(est_probs)
            print(f"\nestimate_B_lms:\n{estimate_B_lms/estimate_B_lms[0,0]}")
            print(f"sample_B_lms:\n{sample_B_lms}")
            
            print("MSE of probs after opt = ", np.mean((est_probs - angles_probs)**2))
            assert np.all(est_probs >= 0)
            assert np.mean((est_probs - angles_probs)**2) < 10**-4
    assert False
